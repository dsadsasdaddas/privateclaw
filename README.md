...
- **Subagent Capability Runtime**: `spawn_agent`, profiles, narrowed execution context, broker, audit log, and tool registry.
...
- 提供 **subagent 权限运行时**：`spawn_agent`、profile、收窄后的 execution context、broker、audit log 与工具注册表。
...
The current runtime routes tool visibility and execution through `CapabilityBroker`, and `spawn_agent` can delegate work to constrained researcher/coder/tester profiles:

```text
SubAgent = same AgentLoop/Kernel + AgentProfile + narrowed ExecutionContext
```

当前运行时的工具可见性和工具执行已经统一经过 `CapabilityBroker`，并且 `spawn_agent` 可以把任务委派给受限的 researcher/coder/tester profile：
